...
export function sortPoints(points, canvas) { // eslint-disable-line
  // How it works?
  const center = [0, 0];
  let A = null;
  let B = null;
...
export function genetateTransformMatrix(rect, dstBounds, transformMatrix, pad = 0) {
...
export function calcIntegralSum(img, x, y, w, h) {
  const a = img.get(y + h, x + w, 0);
  const b = y > 0 ? img.get(y - 1, x + w, 0) : 0;
  const c = x > 0 ? img.get(y + h, x - 1, 0) : 0;
  const d = (y > 0 && x > 0) ? img.get(y - 1, x - 1, 0) : 0;

  return (a - b - c) + d;
}

export function calcHAARFeature(img, feature, size, dx, dy, dStep) {
  let sum = 0;
