...
export default class Tensor extends GraphNode {
  ...
  constructor(dtype, shape, data, stride, offset = 0) {
    ...
    utils.assert(utils.isValidShape(this.shape), 'Shape is not valid');
    if (stride) {
      utils.assert(utils.isValidShape(stride), 'Stride is not valid');
      utils.assert(this.shape.length === stride.length, 'Stride length should be equal to shape length');
    }
    utils.assert(typeof offset === 'number' && offset % 1 === 0, `Offset should be integer, but got ${offset}`);
  ...
  assign(data) {
    ...
    utils.assert(nextDtype === this.dtype, `Different dtypes assigned: \n   expected - ${this.dtype} \n   actual - ${nextDtype}`);
    utils.assert(nextLength === this.size + this.offset, `Different sizes assigned: \n   expected - ${this.size + this.offset} \n   actual - ${nextLength}`);
